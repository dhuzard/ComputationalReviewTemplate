...
| 5 | **actor** | DATAML | `comprev-dataml-phases` | canonical section evidence packages | findings assigned, source records/cite keys attached, v1 schema emitted |
| 5V | **validator** | DATAML | `comprev-curation-validator` | `gate_evidence_curated.json` | schema-valid packages, no duplicates, anti-compression ≥75%, conflicts assigned |
...
| 19V | **validator** | DATAML | `comprev-myst-validator` | validation report | build passes, zero orphans, Evidence Explorer declaration/loader contract and `REVIEW_COMPATIBILITY_REPORT_VALID` re-checked after fixes, `FORBIDDEN_LEXICON` repo-wide (`content/*.md content/*.yml manuscript.tex`) |
...
| 20V | **validator** | DATAML | `comprev-myst-validator` | `gate_repository_push.json` | fresh clone builds, files match, Evidence Explorer loader and `scripts/review-compatibility-report.mjs validate` match the pushed artifact, **`METHODS_LEDGER_FRESH`** (M.6 frame count matches live ledger; "All 21 pipeline phases completed" present; zero forbidden stale phrasings — `are scheduled`, `had not yet executed`, etc.), **`AUTHOR_IDENTITY_NOT_PLACEHOLDER`** (no `Human Supervisor`, `Anonymous`, `TBD`, or `<...>` placeholders in `content/authors.yml`, `myst.yml` `authors:`, or any author-byline block). HARD FAIL — the Phase 20a Methods Ledger Refresh in `comprev-dataml-phases.md` is what populates these; this gate verifies the refresh ran. |
| 21 | **validator** | DATAML | `comprev-deploy-polish` | `gate_phase21_deploy_polish.json` | post-deployment UX gate: tier-A static checks against the built Pages-artifact tarball (`NO_FRONTMATTER_LEAK`, `FIGURE_DROPDOWN_COMPLETE`, `FIGURE_ASSETS_RESOLVE`, `INTERNAL_LINK_HEALTH`, `PLUGIN_DATA_BOUND`, `EVIDENCE_EXPLORER_STATUS_RENDERED`, **`REVIEW_COMPATIBILITY_REPORT_VALID`** (`node scripts/review-compatibility-report.mjs validate` runs against the built artifact), `DIRECTIVE_RENDERED_OK`, `FORBIDDEN_LEXICON_DEPLOYED`, `AUTHOR_IDENTITY_DEPLOYED`); tier-B live-URL checks (`PER_PAGE_HTTP_200`, `EXTERNAL_LINK_HEALTH`) gated by `DEPLOY_ACCESSIBLE` — fall through to a manual user checklist when the deploy URL is unreachable from the sandbox. ONLY phase the coordinator may open after Phase 20 PASS; one targeted fix iteration max, then `ask_user` escalation. |
